...
export default function Cultural() {
  ...
  const fetchTeams = async (page=1) => {
    try {
      setLoading(true);
      const res = await reportsAPI.getTeams({ type: 'cultural', page, limit: PAGE_SIZE });
      setTeams(res.data.items || res.data.teams || []);
      setTeamsTotal(res.data.total || 0);
    } catch (e) {
      console.error('Failed to fetch cultural teams', e);
  ...
  const fetchIndividuals = async (page=1) => {
    try {
      setLoading(true);
      const res = await reportsAPI.getIndividuals({ type: 'cultural', page, limit: PAGE_SIZE });
      setIndividuals(res.data.items || res.data.individuals || []);
      setIndividualsTotal(res.data.total || 0);
    } catch (e) {
      console.error('Failed to fetch cultural individuals', e);
    } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-8 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-3xl font-black text-gray-800">Cultural</h1>
